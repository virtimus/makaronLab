...
import wq.sidePanel.sp_math as m
import wq.sidePanel.sp_helpers as h
...
class SidePanel(qtw.QScrollArea):
    ...
    def initialHandlerSize(self):
        if (self._side == wq.direction.LEFT or self._side == wq.direction.RIGHT):
            return qtc.QSize(25, 240)
        if (self._side == wq.direction.TOP or self._side == wq.direction.DOWN):
    ...
    def __init__(self,parent:qtw.QWidget,side=wq.direction.LEFT):
